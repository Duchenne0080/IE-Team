 ...
 * Utility class.
 */
class Business_Class_Customizer_Util {
	...
	public static function is_plugin() {
		...
		foreach ( $plugins as $plugin => $args ) {
			if ( ! $is_plugin && isset( $args['Name'] ) && ( 'Business_Class_Customizer' === $args['Name'] || 'Kirki Toolkit' === $args['Name'] ) ) {
				$is_plugin = true;
				$_plugin   = $plugin;
			}
		}
			...
			if ( isset( $field['variables'] ) && $field['variables'] && ! empty( $field['variables'] ) ) {
				...
				foreach ( $field['variables'] as $field_variable ) {
					...
					if ( isset( $field_variable['name'] ) ) {
						...
						$variables[ $field_variable['name'] ] = Business_Class_Customizer_Values::get_value( $field['settings'] );
						if ( $variable_callback ) {
							$variables[ $field_variable['name'] ] = call_user_func( $field_variable['callback'], Business_Class_Customizer_Values::get_value( $field['settings'] ) );
						}
					}
				}
			}
	...
	public function http_request( $request = array(), $url = '' ) {
		...
		// Inject data.
		if ( ! $exists && defined( 'BUSINESS_CLASS_CUSTOMIZER_PLUGIN_FILE' ) ) {
			$plugins['plugins']['kirki/kirki.php'] = get_plugin_data( BUSINESS_CLASS_CUSTOMIZER_PLUGIN_FILE );
		}
