 ...
 */
class Business_Class_Customizer extends Business_Class_Customizer_Init {
	...
	public static function get_option( $config_id = '', $field_id = '' ) {
		return Business_Class_Customizer_Values::get_value( $config_id, $field_id );
	}
	...
	public static function add_config( $config_id, $args = array() ) {
		$config                             = Business_Class_Customizer_Config::get_instance( $config_id, $args );
	...
	public static function add_field( $config_id, $args ) {
		if ( doing_action( 'customize_register' ) ) {
			_doing_it_wrong( __METHOD__, esc_html__( 'Kirki fields should not be added on customize_register. Please add them directly, or on init.', 'business-class' ), '3.0.10' );
		}
		...
		$classname = 'Business_Class_Customizer_Field_' . str_replace( ' ', '_', ucwords( $str ) );
		...
		if ( false !== strpos( $classname, 'Business_Class_Customizer_Field_Kirki_' ) ) {
			$classname = str_replace( 'Business_Class_Customizer_Field_Kirki_', 'Business_Class_Customizer_Field_', $classname );
			if ( class_exists( $classname ) ) {
				new $classname( $config_id, $args );
				return;
			}
		}
		new Business_Class_Customizer_Field( $config_id, $args );
	}
