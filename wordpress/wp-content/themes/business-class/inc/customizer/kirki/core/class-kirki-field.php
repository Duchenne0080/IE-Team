 ...
 */
class Business_Class_Customizer_Field {
	 ...
	 *
	 * @see Business_Class_Customizer_Config
	 * @access protected
	 * @var string
	 */
	 ...
	 * Then it adds the field to Business_Class_Customizer::$fields.
	 *
	 * @access public
	 * @param string $config_id    The ID of the config we want to use.
	 *                             Defaults to "global".
	 *                             Configs are handled by the Business_Class_Customizer_Config class.
	...
	public function __construct( $config_id = 'global', $args = array() ) {
		...
		if ( isset( $args['setting'] ) && ! empty( $args['setting'] ) && ( ! isset( $args['settings'] ) || empty( $args['settings'] ) ) ) {
			...
			_doing_it_wrong( __METHOD__, sprintf( esc_html__( 'Typo found in field %s - setting instead of settings.', 'business-class' ), esc_html( $args['settings'] ) ), '3.0.10' );
		...
		if ( is_array( $config_id ) && empty( $args ) ) {
			...
			_doing_it_wrong( __METHOD__, sprintf( esc_html__( 'Config not defined for field %1$s - See %2$s for details on how to properly add fields.', 'business-class' ), esc_html( $args['settings'] ), 'https://kirki.org/docs/getting-started/fields.html' ), '3.0.10' );
			$args      = $config_id;
			$config_id = 'global';
		}
		...
		$config_defaults = ( isset( Business_Class_Customizer::$config['global'] ) ) ? Business_Class_Customizer::$config['global'] : array();
		if ( 'global' !== $this->kirki_config && isset( Business_Class_Customizer::$config[ $this->kirki_config ] ) ) {
			$config_defaults = Business_Class_Customizer::$config[ $this->kirki_config ];
		}
	...
	protected function set_field() {
		...
		Business_Class_Customizer::$fields[ $this->settings ] = $args;
	...
	protected function set_partial_refresh() {
		...
		foreach ( $this->partial_refresh as $id => $args ) {
			if ( ! is_array( $args ) || ! isset( $args['selector'] ) || ! isset( $args['render_callback'] ) || ! is_callable( $args['render_callback'] ) ) {
				/* translators: %1$s represents the field ID where the error occurs. */
				_doing_it_wrong( __METHOD__, sprintf( esc_html__( '"partial_refresh" invalid entry in field %s', 'business-class' ), esc_html( $this->settings ) ), '3.0.10' );
	...
	protected function set_output() {
		...
		if ( ! is_array( $this->output ) ) {
			...
			_doing_it_wrong( __METHOD__, sprintf( esc_html__( '"output" invalid format in field %s. The "output" argument should be defined as an array of arrays.', 'business-class' ), esc_html( $this->settings ) ), '3.0.10' );
			$this->output = array(
				array(
					'element' => $this->output,
				),
