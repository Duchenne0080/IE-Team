 ...
 */
final class Business_Class_Customizer_Modules_Webfonts_Embed {
	...
	/**
	 * The Business_Class_Customizer_Modules_Webfonts object.
	 *
	 * @access protected
	 * @since 3.0.0
	 * @var object
	 ...
	 * @param object $webfonts    The Business_Class_Customizer_Modules_Webfonts object.
	 * @param object $googlefonts The Business_Class_Customizer_Fonts_Google object.
	...
	public function the_css() {
		foreach ( $this->fonts_to_load as $font ) {
			...
			if ( ! $contents ) {
				...
				$contents = Business_Class_Customizer_Fonts_Helper::get_remote_url_contents(
					$url,
					array(
						'headers' => array(
							/**
	...
	private function use_local_files( $css ) {
		...
		foreach ( $matches as $match ) {
			if ( 0 === strpos( $match, 'https://fonts.gstatic.com' ) ) {
				$new_url = Business_Class_Customizer_Fonts_Helper::download_font_file( $match );
				if ( $new_url ) {
