...
/**
 * The Business_Class_Customizer_Modules_CSS object.
 */
class Business_Class_Customizer_Modules_CSS {
	...
	protected function __construct() {
		$class_files = array(
			'Business_Class_Customizer_Modules_CSS_Generator'               => '/class-kirki-modules-css-generator.php',
			'Business_Class_Customizer_Output'                              => '/class-kirki-output.php',
			'Business_Class_Customizer_Output_Field_Background'             => '/field/class-kirki-output-field-background.php',
			'Business_Class_Customizer_Output_Field_Image'                  => '/field/class-kirki-output-field-image.php',
			'Business_Class_Customizer_Output_Field_Multicolor'             => '/field/class-kirki-output-field-multicolor.php',
			'Business_Class_Customizer_Output_Field_Dimensions'             => '/field/class-kirki-output-field-dimensions.php',
			'Business_Class_Customizer_Output_Field_Typography'             => '/field/class-kirki-output-field-typography.php',
			'Business_Class_Customizer_Output_Property'                     => '/property/class-kirki-output-property.php',
			'Business_Class_Customizer_Output_Property_Background_Image'    => '/property/class-kirki-output-property-background-image.php',
			'Business_Class_Customizer_Output_Property_Background_Position' => '/property/class-kirki-output-property-background-position.php',
			'Business_Class_Customizer_Output_Property_Font_Family'         => '/property/class-kirki-output-property-font-family.php',
		);
	...
	public function init() {
		...
		Business_Class_Customizer_Modules_Webfonts::get_instance();
	...
	public function enqueue_styles() {
		...
		wp_enqueue_style(
			...
			array(),
			BUSINESS_CLASS_CUSTOMIZER_VERSION
		);
	}
	...
	public function print_styles() {
		...
		$configs = Business_Class_Customizer::$config;
	...
	public static function loop_controls( $config_id ) {
		...
		// Get an instance of the Business_Class_Customizer_Modules_CSS_Generator class.
		// This will make sure google fonts and backup fonts are loaded.
		Business_Class_Customizer_Modules_CSS_Generator::get_instance();

		$fields = Business_Class_Customizer::$fields;
		$css    = array();
		...
		foreach ( $fields as $field ) {
				...
				if ( ! empty( $field['required'] ) ) {
					...
					foreach ( $field['required'] as $requirement ) {
						if ( isset( $requirement['setting'] ) && isset( $requirement['value'] ) && isset( $requirement['operator'] ) ) {
							$controller_value = Business_Class_Customizer_Values::get_value( $config_id, $requirement['setting'] );
							if ( ! Business_Class_Customizer_Helper::compare_values( $controller_value, $requirement['value'], $requirement['operator'] ) ) {
								$valid = false;
							}
						}
					}
			...
			if ( isset( $field['output'] ) && ! empty( $field['output'] ) ) {
				$css = Business_Class_Customizer_Helper::array_replace_recursive( $css, Business_Class_Customizer_Modules_CSS_Generator::css( $field ) );

				// Add the globals.
				if ( isset( self::$css_array[ $config_id ] ) && ! empty( self::$css_array[ $config_id ] ) ) {
					Business_Class_Customizer_Helper::array_replace_recursive( $css, self::$css_array[ $config_id ] );
				}
			}
		}
		...
		if ( is_array( $css ) ) {
			return Business_Class_Customizer_Modules_CSS_Generator::styles_parse( Business_Class_Customizer_Modules_CSS_Generator::add_prefixes( $css ) );
		}
	}
