...
if ( ! function_exists( 'business_class_customizer_show_upgrade_notification' ) ) :
	/**
	 ...
	 */
	function business_class_customizer_show_upgrade_notification( $plugin_data, $response ) {
		...
		if ( isset( $response->upgrade_notice ) && strlen( trim( $response->upgrade_notice ) ) > 0 ) : ?>
			...
			<div class="kirki-upgrade-notification">
				<strong><?php esc_html_e( 'Important Upgrade Notice:', 'business-class' ); ?></strong>
...
add_action( 'in_plugin_update_message-' . plugin_basename( __FILE__ ), 'business_class_customizer_show_upgrade_notification', 10, 2 );
